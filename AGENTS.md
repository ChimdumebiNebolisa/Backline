...
- **Testcontainers base classes use static initializer pattern**: Both `persistence.PostgresTestBase` and `PostgresWorkerTestBase` start the Testcontainers container via `static {}` (not `@Container`/`@Testcontainers`) to avoid Spring context caching conflicts. Do not switch to `@Container`. If Docker is unavailable, the container start is caught and tests are skipped via JUnit 5 `Assumptions` in `@BeforeAll`.
