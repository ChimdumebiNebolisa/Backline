...
- **Testcontainers base classes use static initializer pattern**: API tests use a single `support.PostgresTestBase` (backed by `PostgresTestContainers`). Worker tests use `PostgresWorkerTestBase`. Both start the Testcontainers container via `static {}` (not `@Container`/`@Testcontainers`) to avoid Spring context caching conflicts. Do not switch to `@Container`. If Docker is unavailable, the container start is caught and tests are skipped via JUnit 5 `Assumptions` in `@BeforeAll` (or fail when `CI=true`).
